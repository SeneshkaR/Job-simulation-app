...
const applyOriginalCareerCorrections = (career) => {
  if (career.id === "nurse") {
    ...
    triage.items.find((item) => item.id === "d").correctRank = 1;
    triage.items.find((item) => item.id === "a").correctRank = 2;
    triage.explanation = "Use airway, breathing, and circulation first: falling oxygen is the immediate priority, followed by possible cardiac ischemia, post-operative pain, and the stable admission.";
  ...
  if (career.id === "data-scientist") {
    ...
    career.quickTrial.exercises[0].options.find((option) => option.id === "d").feedback = "Ignoring missingness can bias results and hide a broken data process.";
  }
...
const applyScopeAndTextCorrections = (career) => {
  ...
  if (career.id === "real-estate-agent") {
    ...
    career.quickTrial.lecture.sections.find((section) => section.type === "myth-vs-reality").reality = "Net commission varies by agreement, brokerage split, expenses, taxes, and whether a transaction closes.";
  }
