...
import { View, Text, StyleSheet, FlatList, TouchableOpacity, ScrollView, TextInput, Platform, Image } from 'react-native';
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.categoryScroll}>
        <CategoryChip label="All" active={selectedCategory === 'All'} onPress={() => setSelectedCategory('All')} colors={colors} />
        {categories.map(cat => (
          <CategoryChip key={cat} label={cat} active={selectedCategory === cat} onPress={() => setSelectedCategory(cat)} colors={colors} />
        ))}
      </ScrollView>
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          const image = getCareerImage(item.id);
          const grad = careerColors[item.id]?.gradient || colors.gradientPrimary;
          return (
              ...
              <View style={styles.careerCard}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <View style={styles.tagRow}>
                    ...
                    <View style={[styles.tag, { backgroundColor: item.demandLevel === 'high' ? `${colors.accentGreen}22` : item.demandLevel === 'medium' ? `${colors.accentYellow}22` : `${colors.accentPink}22` }]}>
...
function CategoryChip({ label, active, onPress, colors }) {
...
function FilterPill({ icon, label, active, onPress, colors }) {
...
const makeStyles = (colors) => StyleSheet.create({
  ...
  careerThumb: { width: 60, height: 60, borderRadius: borderRadius.md, backgroundColor: colors.surfaceLight },
...
const makeChipStyles = (colors) => StyleSheet.create({
...
const makePillStyles = (colors) => StyleSheet.create({
