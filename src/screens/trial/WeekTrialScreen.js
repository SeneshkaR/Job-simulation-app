...
export default function WeekTrialScreen({ route, navigation }) {
  ...
  const savedTrial = state.trials.active?.careerId === careerId && state.trials.active?.trialType === 'WeekTrial'
    ? state.trials.active
    : null;
  const savedProgress = savedTrial?.progress || {};

  const [phase, setPhase] = useState(savedProgress.phase || 'intro'); // intro | day | done
  const [dayIdx, setDayIdx] = useState(savedProgress.dayIdx || 0);
  const [energy, setEnergy] = useState(savedProgress.energy ?? 100);
  const [stress, setStress] = useState(savedProgress.stress ?? 15);
  const [projectProgress, setProjectProgress] = useState(savedProgress.projectProgress || 0);
  const [teamMood, setTeamMood] = useState(savedProgress.teamMood ?? 70);
  const [events, setEvents] = useState(savedProgress.events || []); // events happening today
  const [handledEvents, setHandledEvents] = useState(savedProgress.handledEvents || []);
  const [popup, setPopup] = useState(savedProgress.popup || null);
  const [actions, setActions] = useState(savedProgress.actions || []);
  const popupAnim = useRef(new Animated.Value(savedProgress.popup ? 1 : 0)).current;
  ...
  useEffect(() => {
    if (!savedTrial) {
      startTrial({
        ...
        progress: {
          phase: 'intro', dayIdx: 0, energy: 100, stress: 15,
          projectProgress: 0, teamMood: 70, events: [], handledEvents: [], popup: null, actions: [],
        },
      });
    }
  }, []);
  ...
  useEffect(() => {
    updateTrial({
      progress: { phase, dayIdx, energy, stress, projectProgress, teamMood, events, handledEvents, popup, actions },
    });
  }, [phase, dayIdx, energy, stress, projectProgress, teamMood, events, handledEvents, popup, actions]);
      ...
      <ScrollView contentContainerStyle={{ padding: spacing.xxl, paddingBottom: 120 }}>
        ...
        {events.map(ev => {
          ...
          return (
            ...
            >
              <View style={[styles.evIconWrap, { backgroundColor: `${eventColor(ev.type, colors)}22` }]}>
                <Ionicons name={eventIcon(ev.type)} size={20} color={eventColor(ev.type, colors)} />
              </View>
              <View style={{ flex: 1 }}>
