...
export default function DayTrialScreen({ route, navigation }) {
  ...
  const savedTrial = state.trials.active?.careerId === careerId && state.trials.active?.trialType === 'DayTrial'
    ? state.trials.active
    : null;
  const savedProgress = savedTrial?.progress || {};

  const [step, setStep] = useState(savedProgress.step ?? -1); // -1 intro, then index into schedule
  const [energy, setEnergy] = useState(savedProgress.energy ?? 100);
  const [stress, setStress] = useState(savedProgress.stress ?? 20);
  const [completed, setCompleted] = useState(savedProgress.completed || []);
  const [actions, setActions] = useState(savedProgress.actions || []);
  const [activeEvent, setActiveEvent] = useState(savedProgress.activeEvent || null);
  ...
  useEffect(() => {
    if (!savedTrial) {
      startTrial({
        ...
        progress: { step: -1, energy: 100, stress: 20, completed: [], actions: [], activeEvent: null },
      });
    }
  }, []);

  useEffect(() => {
    updateTrial({ progress: { step, energy, stress, completed, actions, activeEvent } });
  }, [step, energy, stress, completed, actions, activeEvent]);
  ...
  const handleTask = (idx, choice) => {
    ...
    const nextStress = Math.min(100, Math.max(0, stress + dStress));
    const nextEnergy = Math.min(100, Math.max(0, energy + dEnergy));
    const nextAction = { taskId: idx, taskTitle: item.title, choice, score };
    ...
    else finishDay(nextActions, nextEnergy, nextStress);
  };
